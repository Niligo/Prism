...
public class DragItem {
    ...
    void startDrag(View startFromView, float touchX, float touchY) {
        ...
        float startX = ViewCompat.getX(startFromView) - (mDragView.getMeasuredWidth() - startFromView.getMeasuredWidth()) / 2 + mDragView
                .getMeasuredWidth() / 2;
        float startY = ViewCompat.getY(startFromView) - (mDragView.getMeasuredHeight() - startFromView.getMeasuredHeight()) / 2 + mDragView
                .getMeasuredHeight() / 2;
    ...
    void endDrag(View endToView, AnimatorListenerAdapter listener) {
        ...
        float endX = ViewCompat.getX(endToView) - (mDragView.getMeasuredWidth() - endToView.getMeasuredWidth()) / 2 + mDragView
                .getMeasuredWidth() / 2;
        float endY = ViewCompat.getY(endToView) - (mDragView.getMeasuredHeight() - endToView.getMeasuredHeight()) / 2 + mDragView
                .getMeasuredHeight() / 2;
    ...
    void updatePosition() {
        if (mCanDragHorizontally) {
            ViewCompat.setX(mDragView, mPosX + mOffsetX + mAnimationDx - mDragView.getMeasuredWidth() / 2);
        }

        ViewCompat.setY(mDragView, mPosY + mOffsetY + mAnimationDy - mDragView.getMeasuredHeight() / 2);
